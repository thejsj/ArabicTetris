...
public class letter {
	  ...
	  letter(int TletterId, int TletterType, int Tposition, PApplet p) {
	    ...
	    float FR = ((ArabicTetris) parent).frameRate/5;
	    
	    y = ((ArabicTetris) parent).baseline - hgt + ((int) parent.random(-FR,FR)) * ((ArabicTetris) parent).blockHeight;
	  }
	  ...
	  void update() {
	    ...
	    parent.translate( letterPosition, (float) (((ArabicTetris) parent).baseline - ( hgt * 0.15 )));
		    ...
		    parent.text(((ArabicTetris) parent).alphabetArray[letterId][1],0,-200);
	        ...
	        if (rotation == 0 && y == targetYposition 
	          ...
	          if (((ArabicTetris) parent).currentLetter >= ((ArabicTetris) parent).letters.size()) {
	            ...
	            // You Win!
	      		...
	      		// Remove First Menu Option
	      		
	        	  ((ArabicTetris) parent).buttons.remove(0);

	        	  ((ArabicTetris) parent).gameState = 3;
	        	  
	        	  ((ArabicTetris) parent).endGame(((ArabicTetris) parent).gameState);
	          ...
	          // Game over baby, you screwed up!
	        	...
	        	((ArabicTetris) parent).checkHighScore();
